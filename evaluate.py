...
def colorize_instances(instance_masks):
    ...
    width, height, _ = instance_masks.shape
...
def colorize_class_mask(class_mask_int, class_num):
    ...
    width, height, _ = class_mask_int.shape
...
def eval_pair(model, pair, params):
    ...
    class_mask_pred      = outputs[0, :, :, :class_num]
    prev_class_mask_pred = outputs[0, :, :, class_num:(class_num*2)]
    embedding_pred       = outputs[0, :, :, (class_num*2):(class_num*2 + embedding_dim)]
    prev_embedding_pred  = outputs[0, :, :, (class_num*2 + embedding_dim):((class_num*2 + embedding_dim*2))]
    ...
    combined_class_mask_pred[:, output_size:] = prev_class_mask_pred
    ...
    identity_mask_gt_color        = colorize_instances(identity_mask_gt)
    prev_identity_mask_gt_color   = colorize_instances(prev_identity_mask_gt)
    ...
    class_mask_pred_color         = colorize_class_mask(class_mask_pred, class_num)
    prev_class_mask_pred_color    = colorize_class_mask(prev_class_mask_pred, class_num)
